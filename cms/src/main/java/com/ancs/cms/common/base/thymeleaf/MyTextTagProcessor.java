 ...
 *
 */
public final class MyTextTagProcessor extends AbstractAttributeTagProcessor implements IPostProcessor {

	public static final int PRECEDENCE = 1300000;
	...
	protected MyTextTagProcessor(final TemplateMode templateMode, final String dialectPrefix) {
	...
	protected final void doProcess(final ITemplateContext context, final IProcessableElementTag tag,
		...
		for (int i = 0; i < assignationValuesLen; i++) {
			...
			final String newAttributeName = "th:"+(leftValue == null ? null : leftValue.toString());
			if (StringUtils.isEmptyOrWhitespace(newAttributeName)) {
				throw new TemplateProcessingException(
						"Attribute name expression evaluated as null or empty: \"" + leftExpr + "\"");
			}

			if (getTemplateMode() == TemplateMode.HTML && this.modificationType == ModificationType.SUBSTITUTION
					&& ArrayUtils.contains(StandardConditionalFixedValueTagProcessor.ATTR_NAMES, newAttributeName)) {
				// Attribute is a fixed-value conditional one, like "selected",
				// which can only
				...
				// Attribute is a "normal" attribute, not a fixed-value
				// conditional one - or we are not just replacing

				final String newAttributeValue = HtmlEscape
						.escapeHtml4Xml(rightValue == null ? null : rightValue.toString());
				if (newAttributeValue == null || newAttributeValue.length() == 0) {

					if (this.modificationType == ModificationType.SUBSTITUTION) {
						// Substituting by a no-value will be equivalent to
						// simply removing
						structureHandler.removeAttribute(newAttributeName);
					}
					...
					if (this.modificationType == ModificationType.SUBSTITUTION || !tag.hasAttribute(newAttributeName)
